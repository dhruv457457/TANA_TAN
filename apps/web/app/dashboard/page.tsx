    ...
    <div className="min-h-screen bg-black">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-10">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <h1 className="text-3xl font-black text-white tracking-tight mb-2">Portfolio Dashboard</h1>
          <p className="text-white/40">Your active positions across 20+ protocols via LI.FI Earn</p>
        </motion.div>
        ...
        >
          <input
            ...
            className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-violet-500/50 transition-colors font-mono"
          />
          <button
            onClick={handleLoad}
          ...
          {address && (
            <button
              ...
              className="px-4 py-3 rounded-xl border border-white/10 hover:border-white/20 text-white/50 hover:text-white text-sm transition-colors shrink-0"
        ...
        {/* Note about indexing delay */}
        {address && (
          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-xs text-white/30 mb-4 -mt-4"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {(vaults ?? []).slice(0, 6).map((vault, i) => (
              <motion.div
                ...
                className="flex items-center gap-3 p-4 rounded-xl border border-white/5 bg-white/[0.03] hover:bg-white/[0.06] transition-colors"
              >
