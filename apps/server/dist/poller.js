...
async function runPoll() {
    ...
    try {
        ...
        for (const strategy of strategies) {
            ...
            console.log(`[Poller:${runId}] Strategy ${strategy._id} (${strategy.protocol}) — ${pendingCount} followers pending`);
            try {
                const results = await executeStrategy(strategy._id.toString(), BACKEND_ADDRESS, lastTriggeredAt);
                const successes = results.filter((r) => r.status === "success").length;
                totalExecuted += successes;
                console.log(`[Poller:${runId}] Strategy ${strategy._id}: ${successes}/${results.length} succeeded`);
            }
            catch (err) {
        ...
        console.log(`[${new Date().toISOString()}] [Poller:${runId}] Done. Executed: ${totalExecuted}, Skipped: ${totalSkipped}`);
    }
    catch (err) {
